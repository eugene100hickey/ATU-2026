...
- two packages of note; `pdftools` and `tabulapdf`

- `tabulapdf` has an `extract_tables()` function.

    - good to specify the specific page or location in the pdf of the table you want
    
    -  I (pretty much) always include a `output = "tibble"` in the call to `extract_tables()`
...
`here(system.file(package="tabulapdf"), "examples", "mtcars.pdf") |> `
`extract_tables(output = "tibble", pages = 1) |> `
`pluck(1)`

:::

## installing `tabulapdf`

::: {style="font-size: 80%; list-style-position: outside; padding-left: 0em;"}

- Need **Java** installed (I think), and R needs to know about it.
...
- then `install.package("tabulapdf")`
...
extract_tables(organ_transplant, output = "tibble", pages = 2) |> pluck(1) |> gt::gt()
```
...
Find the [*Annual*]{.underline},  [*Proportion of youth (20-24 years) having attained ISCED 3 level, total (%)*]{.underline} in [*Palestine*]{.underline} in [*2014*]{.underline}. __[40%]__

(@) Install the `BiocManager` package (`install.packages("BiocManager")`). Install the `rentrez` and `seqinr` packages from `Bioconductor` (e.g. `BiocManager::install("rentrez")`). Find the [NCBI]() accession numbers for the *BRCA1* cancer gene using `z <- entrez_search(db = "nuccore", term = "brca1")`. This returns a list of 5 elements, the first of which (`z[[1]]`) will be the relevant accession numbers. Take the first of these accession numbers and fetch the gene sequence using `entrez_fetch(db = "nuccore", id = "my_accession_number", rettype = "fasta")`.

(@)  Install **Selector Gadget** as a Chrome extension. Go to the [RTE  website](https://www.rte.ie/). Find the *css* selector for the top navigation bar (where it says *"News Sport Entertainment..."*). __[".primary-bar__nav__link""]__

(@) Go to the wikipedia page for the [English Premier League Season 2024/2025](https://en.wikipedia.org/wiki/2024%E2%80%9325_Premier_League){target="_blank"}. Find the css selector for the team that qualified for the _Europa League_ (should be something like "**tr:nth-child(6) .infobox-data a:nth-child(1)**"). Use `read_html()`, `html_elements()`, and `html_text2()` from the `rvest` package to find the team that qualified. __[Aston Villa]__
